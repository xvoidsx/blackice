...
echo "*** blackice.mv3: Creating extension"
...
UBOL_DIR="dist/build/blackice.$PLATFORM"
...
if [ -n "$UBO_VERSION" ]; then
    ...
    UBO_DIR=$(mktemp -d)
    echo "*** blackice.mv3: Fetching uBO $UBO_VERSION from $UBO_REPO into $UBO_DIR"
    cd "$UBO_DIR"
    git init -q
...
echo "*** blackice.mv3: Copying common files"
cp -R "$UBO_DIR"/src/css/fonts/Inter "$UBOL_DIR"/css/fonts/
cp "$UBO_DIR"/src/css/themes/default.css "$UBOL_DIR"/css/
# blackice: our nightshadeNeon token overrides load after default.css
cp "$UBO_DIR"/src/css/themes/blackice.css "$UBOL_DIR"/css/
...
echo "*** blackice.mv3: Copying mv3-specific files"
...
echo "*** blackice.mv3: Generating rulesets"
...
if [ -n "$BEFORE" ]; then
    echo "*** blackice.mv3: salvaging rule ids to minimize diff size"
...
echo "*** blackice.$PLATFORM: extension ready"
...
if [ "$PLATFORM" = "edge" ]; then
    ...
    echo "*** blackice.edge: Modify reference implementation for Edge compatibility"
...
if [ "$FULL" = "yes" ]; then
    ...
    echo "*** blackice.mv3: Creating publishable package..."
    UBOL_PACKAGE_NAME="blackice_$TAGNAME.$PLATFORM.$EXTENSION"
